...
Le présent guide a été mis à jour en 2025 à la suite de l'implémentation d'ArchivesSpace dans notre flux de travail.
...
* **Membre du personnel qui reçoit le transfert (généralement l'Archiviste Numérique): Réception**
   * Créer l'acquisition dans ArchivesSpace.
...
* **Archiviste Numérique : ingestion**
   ...
   * Supprimer toutes copies supplémentaires de fichiers dans le dossier _Processing_, _Digital Shipping Space_ ou sur les postes de travail BitCurator.
...
* **Technicien/ne aux Archives Numériques: Stabilisation**
  ...
  * Lancer une analyse antivirus sur l'entièreté de l'acquisition.
  * Vérifier les métadonnées, spécifiquement la date de la dernière modification sur chacun des fichiers afin de s'assurer que rien n'a été modifié durant la stabilisation.
  * Comparer l'inventaire du donateur/ de la donatrice avec les fichiers reçus, si disponible.
...
* **Archiviste Numérique : ingestion**
  * Ingérer le SIP dans Archivematica en suivant les [procédures pour l'ingestion des données brutes](#integrer).
  * Supprimer toutes copies supplémentaires dans le dossier _Processing_, _Digital Shipping Space_ ou sur les postes de travail BitCurator.
  * Demander au Technicien/ à la Technicienne aux Archives Numériques de mettre à jour le conteneur de l'acquisition dans ArchivesSpace pour refléter le nouvel emplacement.
...
* **L'équipe de la gestion de la collection (GesCo) : Réception**
   ...
   * SI NON : GesCo doit indiquer dans ArchivesSpace que le versement contient des supports numériques. Le matériel numérique sera ensuite stabilisé par le Technicien ou la Technicienne des Archives Numériques, et le versement-objet sera ajouté dans le fichier Excel du "Backlog Project".
* **Technicien/ne aux Archives Numériques: Stabilisation**
  ...
  * Noter l'emplacement actuel des boites dans lesquelles se retrouvent le versement choisi et aller les chercher à l'aide d'un charriot.
  ...
  * Identifier chaque support avec un identifiant AS unique à l'aide d'un stylo ou d'un crayon à mine, selon la surface du support (ex. AS14900001).
  ...
  * Si ce versement ne va pas être traité tout de suite, empaqueter tout le contenu brut en tant que SIP Archivematica en compressant les fichiers dans un des divers formats d'archive (zip, tar, rar, etc.), sauvegarder le sur le RAID sur un des BitCurator et renommer le SIP en suivant les [procédures pour l'ingestion des données brutes](#integrer).
  * Sinon, vous pouvez empaqueter chaque image disque individuellement dans leur propre SIP en utilisant les [CCA tools](#cca-tools). Sauvegarder tous les SIPs sur le RAID sur l'un des BitCurator et placez-les dans un dossier que vous pourrez renommer en utilisant le numéro du fonds d'archives et une version abrégée du nom du fonds, comme ceci : "AP149_MCHG" où le numéro de fonds d'archives est "AP149" et "MCHG" se réfère à "Minimum Cost Housing Group".
...
Une fois que l'Archiviste au Traitement ou que le Technicien/ la Technicienne aux Archives Numériques ait téléchargé les fichiers et stabilisé tous les supports physiques ou temporaires, l'acquisition doit être ingérée dans le système de préservation numérique du CCA, Archivematica. Le but de cette étape est de conserver et de stocker en toute sécurité une copie des données telles qu'elles sont arrivées au CCA. 
...
Une fois l'acquisition ingérée dans Archivematica, supprimer toute copie supplémentaire des machines BitCurator, Digital Shipping Space, etc. et demander au Technicien/ à la Technicienne aux Archives Numériques de mettre à jour le conteneur de l'acquisition à "Dark Archive".
